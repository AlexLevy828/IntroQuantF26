...
THIS TUTORIAL COMPLETED BY: (Alexander Levy)
...
```{r}
ggplot(candy_rankings) + aes(x= chocolate) # this line makes the space to put the plot, the background
```

```{r}
ggplot(candy_rankings) + 
  aes(x = chocolate) +
  geom_bar()# this says what to put on the axes
```
...
```{r}
sweet_chocolate <- ggplot(candy_rankings)+
...
```{r}
fruit_cheap <- ggplot(candy_rankings) +
...
```{r}
sweet_expensive <- ggplot(candy_rankings) +
...
```{r}
sweet_win <- ggplot(candy_rankings) + 
  aes(x = sugarpercent, y = winpercent) +
  geom_point() +
  geom_smooth(method = "lm")
sweet_win
